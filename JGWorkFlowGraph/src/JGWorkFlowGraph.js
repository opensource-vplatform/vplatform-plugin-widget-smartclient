...
isc.JGWorkFlowGraph.addMethods({
	...
	initTool: function () {
		...
		var lable1 = isc.Label.create({
			...
			width: 28,
			prompt: '放大',
			backgroundImage: '[SKIN]/JGWorkFlowGraph/JGWorkFlowGraph_Background.png',
   ...
   */
	...
	setDefinitionJson: function (definitionJson) {
		var datasource = this.getDataSource();
		datasource.clear();
		var unserializedb = this.createDatasourceFromJson(definitionJson);
		datasource.insertRecords(unserializedb.getAllRecords().toArray())
	},
	/*
	 * 设置线的样式
	 *
